...
export default function AdminDashboard() {
  const [stats, setStats] = useState<Stats>({
    totalNovels: 0, totalAuthors: 0, totalViews: 0,
    totalComments: 0, totalUsers: 0, recentNovels: [],
  });
  ...
  const statCards = [
    ...
    { label: 'Autores', value: stats.totalAuthors, icon: Users, color: 'text-purple-accent' },
    { label: 'Visualizações', value: stats.totalViews, icon: Eye, color: 'text-green-500' },
    { label: 'Comentários', value: stats.totalComments, icon: MessageSquare, color: 'text-blue-500' },
  ];
  ...
  return (
    <AdminLayout>
      <div className="space-y-8">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="font-display">Novels Recentes</CardTitle>
          </CardHeader>
          <CardContent>
            {stats.recentNovels.length === 0 ? (
              <p className="text-muted-foreground text-center py-8">Nenhuma novel cadastrada ainda.</p>
            ) : (
                ...
                {stats.recentNovels.map(novel => (
                  <div key={novel.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      <span className="flex items-center gap-1"><Eye className="w-4 h-4" />{novel.views}</span>
                      <span className="flex items-center gap-1"><Star className="w-4 h-4" />{novel.rating}</span>
                    </div>
                  </div>
                ))}
              </div>
