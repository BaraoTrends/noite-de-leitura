...
export default function AdminAuthors() {
  ...
  const openNew = () => {
    setEditingId(null);
    setForm({ name: '', slug: '', bio: '', avatar_url: '', twitter: '', instagram: '', website: '', is_verified: false, is_active: true });
    setDialogOpen(true);
  };
  ...
  const openEdit = (a: any) => {
    setEditingId(a.id);
    setForm({
      name: a.name, slug: a.slug, bio: a.bio || '', avatar_url: a.avatar_url || '',
      twitter: a.twitter || '', instagram: a.instagram || '', website: a.website || '',
      is_verified: a.is_verified, is_active: a.is_active,
    });
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      await supabase.from('authors').update(data).eq('id', editingId);
      toast({ title: 'Autor atualizado!' });
    } else {
      await supabase.from('authors').insert(data);
      toast({ title: 'Autor criado!' });
    }
    setDialogOpen(false);
    fetchAuthors();
  };
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-3xl text-foreground">Autores</h1>
            <p className="text-muted-foreground mt-1">Gerencie os autores da plataforma</p>
          </div>
          <Button onClick={openNew}><Plus className="w-4 h-4 mr-2" />Novo Autor</Button>
        </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    {author.avatar_url ? (
                      ...
                      <div className="w-12 h-12 rounded-full bg-muted flex items-center justify-center text-lg font-display">
                    ...
                    <div>
                      <div className="flex items-center gap-1">
                        ...
                        {author.is_verified && <CheckCircle className="w-4 h-4 text-primary" />}
                      </div>
