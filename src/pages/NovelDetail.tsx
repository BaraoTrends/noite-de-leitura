...
const NovelDetail = () => {
    ...
    return (
      <Layout>
        <div className="container mx-auto px-4 py-20 text-center">
          <h1 className="font-display text-3xl text-foreground mb-4">Novel não encontrada</h1>
          <Link to="/">
  ...
  const relatedNovels = novels
    .filter((n) => n.id !== novel.id && n.categories.some((c) => novel.categories.includes(c)))
    .slice(0, 4);
    ...
    return (
      <div className="min-h-screen bg-background">
        <div className="fixed top-4 right-4 z-50 flex gap-2">
          <Button variant="outline" size="icon" onClick={() => setFontSize((s) => Math.max(14, s - 2))}>
            <Minus className="w-4 h-4" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => setFontSize((s) => Math.min(28, s + 2))}>
            <Plus className="w-4 h-4" />
          </Button>
          <Button variant="outline" onClick={() => setIsImmersive(false)}>
            Sair
          </Button>
        </div>
        <article 
          className="max-w-3xl mx-auto px-8 py-20 prose prose-invert prose-lg"
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-6 mb-6">
                <div className="flex items-center gap-2">
                  <Rating value={novel.rating} showValue />
                  <span className="text-sm text-muted-foreground">({novel.ratingCount} avaliações)</span>
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Eye className="w-4 h-4" />
                  <span>{formatViews(novel.views)} visualizações</span>
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Clock className="w-4 h-4" />
                  <span>{novel.readTime} min de leitura</span>
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Calendar className="w-4 h-4" />
                  <span>{formatDate(novel.publishDate)}</span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                <Button variant="hero" onClick={() => setIsImmersive(true)}>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-8 p-4 bg-card rounded-xl border border-border">
              ...
              <Button variant="outline" onClick={() => setIsImmersive(true)}>
                Modo Leitura
              </Button>
            </div>
            ...
            {/* Content */}
            <article 
              className="prose prose-invert prose-lg max-w-none bg-card rounded-xl p-8 border border-border"
              style={{ fontSize: `${fontSize}px` }}
            >
              <div 
                className="text-foreground/90 leading-relaxed whitespace-pre-wrap"
            ...
            <div className="mt-12">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="font-display text-2xl font-bold text-foreground">
                  Comentários ({novel.commentsCount})
                </h2>
              </div>
          ...
          <aside className="w-full lg:w-80 flex-shrink-0">
            <div className="lg:sticky lg:top-24 space-y-6">
              ...
              {novel.youtubeVideoId && (
                <div className="bg-card rounded-xl p-5 border border-border">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-display font-semibold text-foreground">Versão Narrada</h3>
                  </div>
                  <div className="aspect-video rounded-lg overflow-hidden">
                    <iframe
                      width="100%"
                      height="100%"
                      src={`https://www.youtube.com/embed/${novel.youtubeVideoId}`}
                      title="Narração"
                      frameBorder="0"
              ...
              {relatedNovels.length > 0 && (
                <div className="bg-card rounded-xl p-5 border border-border">
                  <h3 className="font-display font-semibold text-foreground mb-4">Novels Relacionadas</h3>
                  <div className="space-y-4">
                    {relatedNovels.map((n, index) => (
                      <NovelCard key={n.id} novel={n} index={index} variant="compact" />
                    ))}
                  </div>
                </div>
              )}
